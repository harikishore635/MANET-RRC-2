...
// ============================================================================
// IPC CONFIGURATION - ALL PROCESSES COMMUNICATE VIA MESSAGE QUEUES
// ============================================================================
...
#define MQ_OLSR_TO_RRC "/mq_olsr_to_rrc" // OLSR → RRC
#define MQ_RRC_TO_OLSR "/mq_rrc_to_olsr" // RRC → OLSR
#define MQ_PHY_TO_RRC "/mq_phy_to_rrc"   // PHY → RRC
#define MQ_RRC_TO_PHY "/mq_rrc_to_phy"   // RRC → PHY
...
static uint32_t ipc_request_counter = 0;
...
// Neighbor State Structure (neighbour tx/rx info)
typedef struct
{
    uint16_t nodeID;
...
// RRC Slot allocation table for all 10 slots per frame (0-9)
// Slots 0-7: DU/GU data slots
// Slots 8-9: NC (Network Control) slots
static TDMA_SlotInfo tdma_slot_table[10];
...
// IPC wrapper functions (replace extern API calls)
...
int rrc_send_to_olsr(const void *msg, size_t msg_size);
int rrc_receive_from_olsr(void *msg, size_t msg_size, bool blocking);
int rrc_send_to_phy(const void *msg, size_t msg_size);
int rrc_receive_from_phy(void *msg, size_t msg_size, bool blocking);
...
// Initialize IPC (message queues and shared memory)
int rrc_ipc_init(void)
{
    if (ipc_initialized)
    {
        printf("RRC IPC: Already initialized\n");
        return 0;
    }
    ...
    // Create/Open OLSR message queues
    mq_rrc_to_olsr = mq_open(MQ_RRC_TO_OLSR, O_CREAT | O_WRONLY, 0666, &attr);
    if (mq_rrc_to_olsr == (mqd_t)-1)
    {
        perror("RRC IPC: Failed to open RRC->OLSR queue");
        return -1;
    }

    mq_olsr_to_rrc = mq_open(MQ_OLSR_TO_RRC, O_CREAT | O_RDONLY, 0666, &attr);
    if (mq_olsr_to_rrc == (mqd_t)-1)
    {
        perror("RRC IPC: Failed to open OLSR->RRC queue");
        return -1;
    }

    // Create/Open PHY message queues
    mq_rrc_to_phy = mq_open(MQ_RRC_TO_PHY, O_CREAT | O_WRONLY, 0666, &attr);
    if (mq_rrc_to_phy == (mqd_t)-1)
    {
        perror("RRC IPC: Failed to open RRC->PHY queue");
        return -1;
    }

    mq_phy_to_rrc = mq_open(MQ_PHY_TO_RRC, O_CREAT | O_RDONLY, 0666, &attr);
    if (mq_phy_to_rrc == (mqd_t)-1)
    {
        perror("RRC IPC: Failed to open PHY->RRC queue");
        return -1;
    }

    ipc_initialized = true;
    printf("RRC IPC: Initialized successfully (4 message queues)\\n\");
    return 0;
}
...
// Send message to OLSR
int rrc_send_to_olsr(const void *msg, size_t msg_size)
{
    if (!ipc_initialized || mq_rrc_to_olsr == (mqd_t)-1)
        return -1;

    if (mq_send(mq_rrc_to_olsr, (const char *)msg, msg_size, 0) == -1)
    {
        perror("RRC IPC: Failed to send to OLSR");
        return -1;
    }
    return 0;
}

// Receive message from OLSR
int rrc_receive_from_olsr(void *msg, size_t msg_size, bool blocking)
{
    if (!ipc_initialized || mq_olsr_to_rrc == (mqd_t)-1)
        return -1;

    if (!blocking)
    {
        struct mq_attr attr;
        mq_getattr(mq_olsr_to_rrc, &attr);
        if (attr.mq_curmsgs == 0)
            return 0; // No messages
    }

    ssize_t bytes = mq_receive(mq_olsr_to_rrc, (char *)msg, msg_size, NULL);
    if (bytes == -1)
    {
        if (errno != EAGAIN)
            perror("RRC IPC: Failed to receive from OLSR");
        return -1;
    }
    return bytes;
}
...
int rrc_send_to_phy(const void *msg, size_t msg_size)
{
    if (!ipc_initialized || mq_rrc_to_phy == (mqd_t)-1)
        return -1;

    if (mq_send(mq_rrc_to_phy, (const char *)msg, msg_size, 0) == -1)
    {
        perror("RRC IPC: Failed to send to PHY");
        return -1;
    }
    return 0;
}

// Receive message from PHY
int rrc_receive_from_phy(void *msg, size_t msg_size, bool blocking)
{
    if (!ipc_initialized || mq_phy_to_rrc == (mqd_t)-1)
        return -1;

    if (!blocking)
    {
        struct mq_attr attr;
        mq_getattr(mq_phy_to_rrc, &attr);
        if (attr.mq_curmsgs == 0)
            return 0; // No messages
    }

    ssize_t bytes = mq_receive(mq_phy_to_rrc, (char *)msg, msg_size, NULL);
    if (bytes == -1)
    {
        if (errno != EAGAIN)
            perror("RRC IPC: Failed to receive from PHY");
        return -1;
    }
    return bytes;
}

// IPC wrapper: Get next hop from OLSR (request/response pattern)
uint8_t ipc_olsr_get_next_hop(uint8_t destination_node_id)
{
    if (!ipc_initialized)
    {
        printf("RRC IPC: Not initialized, cannot get next hop\\n\");
        return 0;
    }

    // Send route request to OLSR
    IPC_RouteRequest request;
    request.type = MSG_RRC_ROUTE_REQUEST;
    request.dest_node = destination_node_id;
    request.request_id = ++ipc_request_counter;

    if (rrc_send_to_olsr(&request, sizeof(request)) < 0)
    {
        printf("RRC IPC: Failed to send route request to OLSR\\n\");
        return 0;
    }

    // Wait for response (with timeout)
    IPC_RouteResponse response;
    struct timespec timeout;
    clock_gettime(CLOCK_REALTIME, &timeout);
    timeout.tv_sec += 1; // 1 second timeout

    struct mq_attr old_attr, new_attr;
    mq_getattr(mq_olsr_to_rrc, &old_attr);
    new_attr = old_attr;
    new_attr.mq_flags = O_NONBLOCK;
    mq_setattr(mq_olsr_to_rrc, &new_attr, NULL);

    int retries = 10;
    while (retries-- > 0)
    {
        ssize_t bytes = mq_receive(mq_olsr_to_rrc, (char *)&response, sizeof(response), NULL);
        if (bytes > 0 && response.type == MSG_OLSR_ROUTE_UPDATE &&
            response.request_id == request.request_id)
        {
            mq_setattr(mq_olsr_to_rrc, &old_attr, NULL);
            return response.route_available ? response.next_hop : 0;
        }
        usleep(100000); // 100ms
    }

    mq_setattr(mq_olsr_to_rrc, &old_attr, NULL);
    printf("RRC IPC: Timeout waiting for OLSR route response\\n\");
    return 0;
}

// IPC wrapper: Trigger route discovery in OLSR
void ipc_olsr_trigger_route_discovery(uint8_t destination_node_id)
{
    if (!ipc_initialized)
        return;
    ...
    IPC_RouteRequest request;
    request.type = MSG_RRC_DISCOVERY_TRIGGER;
    request.dest_node = destination_node_id;
    request.request_id = ++ipc_request_counter;

    rrc_send_to_olsr(&request, sizeof(request));
}

// IPC wrapper: Get PHY metrics
void ipc_phy_get_link_metrics(uint8_t node_id, float *rssi, float *snr, float *per)
{
    if (!ipc_initialized || !rssi || !snr || !per)
    {
        if (rssi)
            *rssi = -100.0f;
        if (snr)
    ...
    // Send metrics request to PHY
    IPC_RouteRequest request; // Reuse structure
    request.type = MSG_RRC_METRICS_REQUEST;
    request.dest_node = node_id;
    request.request_id = ++ipc_request_counter;

    if (rrc_send_to_phy(&request, sizeof(request)) < 0)
    {
        *rssi = -100.0f;
        *snr = 0.0f;
        *per = 100.0f;
    ...
    // Wait for response
    IPC_PHYMetrics response;
    struct mq_attr old_attr, new_attr;
    mq_getattr(mq_phy_to_rrc, &old_attr);
    new_attr = old_attr;
    new_attr.mq_flags = O_NONBLOCK;
    mq_setattr(mq_phy_to_rrc, &new_attr, NULL);

    int retries = 10;
    while (retries-- > 0)
    {
        ssize_t bytes = mq_receive(mq_phy_to_rrc, (char *)&response, sizeof(response), NULL);
        if (bytes > 0 && response.type == MSG_PHY_METRICS_UPDATE &&
            response.node_id == node_id)
        {
            *rssi = response.rssi_dbm;
            *snr = response.snr_db;
            *per = response.per_percent;
            mq_setattr(mq_phy_to_rrc, &old_attr, NULL);
            return;
        }
        usleep(100000); // 100ms
    }
...
// IPC wrapper: Check if PHY link is active
bool ipc_phy_is_link_active(uint8_t node_id)
{
    float rssi, snr, per;
    ipc_phy_get_link_metrics(node_id, &rssi, &snr, &per);
    return (rssi > -95.0f && snr > 5.0f);
}

// IPC wrapper: Get packet count from PHY
uint32_t ipc_phy_get_packet_count(uint8_t node_id)
{
    if (!ipc_initialized)
        return 0;
...
// Helper: check if NC slot is currently conflicted (local view)->neighbour table
...
// Check if we can preempt an NC slot based on priority score
static bool rrc_can_preempt_nc_slot_by_score(uint8_t slot, uint32_t my_priority_score, uint16_t my_node_id)
{
    ...
    for (int i = 0; i < neighbor_count; i++)
    {
        if (neighbor_table[i].active && neighbor_table[i].assignedNCSlot == slot)
        {
            ...
            // We can preempt if our priority score is lower (higher priority)
...
// Preempt an NC slot from lower priority user
static void rrc_preempt_nc_slot(uint8_t slot, uint16_t new_owner)
{
    // Find and notify current owner of preemption
    for (int i = 0; i < neighbor_count; i++)
    {
        if (neighbor_table[i].active && neighbor_table[i].assignedNCSlot == slot)
        {
            printf("RRC PRIORITY: Preempting NC slot %u from node %u (new owner: %u)\n",
                   slot, neighbor_table[i].nodeID, new_owner);

            // Clear old assignment
            neighbor_table[i].assignedNCSlot = 0;

            // Generate preemption notification if it was our own slot
            if (neighbor_table[i].nodeID == rrc_node_id)
            {
                printf("RRC PRIORITY: Our NC slot %u was preempted by higher priority node %u\n",
                       slot, new_owner);
...
static uint8_t rrc_assign_nc_slot_by_priority(const NCReservationRequest *request)
{
    ...
    if (candidate_slot > 0 && candidate_slot <= NC_SLOTS_PER_SUPERCYCLE)
    {
        ...
        // Check if we can preempt lower priority reservation
        if (rrc_can_preempt_nc_slot_by_score(candidate_slot, my_priority_score, request->nodeID))
        {
            printf("RRC PRIORITY: Preempting lower priority reservation on slot %u\n", candidate_slot);
            rrc_preempt_nc_slot(candidate_slot, request->nodeID);
            rrc_update_nc_status_bitmap(candidate_slot, true);
            return candidate_slot;
        }
    }
    ...
    for (int t = 0; t < MAX_TRIES; ++t)
    {
        ...
        // Check if we can preempt this slot
        if (rrc_can_preempt_nc_slot_by_score(slot, my_priority_score, request->nodeID))
        {
            printf("RRC PRIORITY: Preempting slot %u for higher priority node %u (score: %u)\n",
                   slot, request->nodeID, my_priority_score);
            rrc_preempt_nc_slot(slot, request->nodeID);
            rrc_update_nc_status_bitmap(slot, true);
            return slot;
        }
    }
...
{
    // Calculate hop count to destination using OLSR via IPC
...
uint8_t rrc_assign_nc_slot_with_multi_relay_priority(uint16_t nodeID, uint32_t packet_count)
{
    ...
    // Try to get hop count from OLSR via IPC for relay traffic
    if (!is_self)
    {
...
bool rrc_should_relay(struct frame *frame)
{
    ...
    // Check if OLSR has route via IPC
...
void rrc_enqueue_relay_packet(struct frame *frame)
{
    ...
    // Get new next hop from OLSR via IPC
    uint8_t new_next_hop = ipc_olsr_get_next_hop(frame->dest_add);
...
int rrc_handle_power_on(void)
{
    ...
    if (rrc_ipc_init() < 0)
    {
        printf("RRC: ERROR - IPC initialization failed\n");
        return -1;
    }
...
int rrc_handle_route_and_slots_allocated(uint8_t dest_node, uint8_t next_hop)
{
    ...
    // Check RRC slot availability and allocate
    if (!rrc_check_slot_available(next_hop, ctx->qos_priority))
    {
        printf("RRC: ⚠️ No slots available for next hop %u\n", next_hop);
    ...
    uint8_t allocated_slot = rrc_allocate_du_gu_slot(next_hop, ctx->qos_priority);
    if (allocated_slot == 255)
    {
        printf("RRC: ERROR - Slot allocation failed for node %u\n", dest_node);
        rrc_transition_to_state(RRC_STATE_IDLE, dest_node);
        rrc_release_connection_context(dest_node);
        return -1;
    }
...
int rrc_handle_route_change(uint8_t dest_node, uint8_t new_next_hop)
{
    ...
    // Trigger route discovery for verification via IPC
...
int rrc_request_transmit_slot(uint8_t dest_node, MessagePriority priority)
{
    ...
    // Check RRC slot availability
    if (!rrc_check_slot_available(ctx->next_hop_id, priority))
    {
        printf("RRC: No transmit slots available for node %u (priority %d)\n", dest_node, priority);
        return -1;
    }

    // Allocate slot for transmission
    uint8_t allocated_slot = rrc_allocate_du_gu_slot(ctx->next_hop_id, priority);
    if (allocated_slot == 255)
    {
        printf("RRC: Slot allocation failed for node %u\n", dest_node);
        return -1;
    }

    printf("RRC: Transmit slot %u allocated for node %u via next hop %u\n",
           allocated_slot, dest_node, ctx->next_hop_id);
    return 0;
}
...
void update_phy_metrics_for_node(uint8_t node_id)
{
    ...
    // Get metrics from PHY layer via IPC
    ipc_phy_get_link_metrics(node_id, &rssi, &snr, &per);
...
void enqueue_to_appropriate_queue(ApplicationMessage *app_msg, uint8_t next_hop_node)
{
    ...
    // Check RRC slot availability and allocate before enqueueing
    if (app_msg->priority != PRIORITY_ANALOG_VOICE_PTT)
    {
        if (!rrc_check_slot_available(next_hop_node, app_msg->priority))
        {
            printf("RRC: ⚠️ No slots available for next hop %u - discarding message\n", next_hop_node);
            rrc_stats.messages_discarded_no_slots++;
        ...
        // Allocate DU/GU slot for this transmission
        uint8_t allocated_slot = rrc_allocate_du_gu_slot(next_hop_node, app_msg->priority);
        if (allocated_slot == 255)
        {
            printf("RRC: ⚠️ Slot allocation failed for next hop %u\n", next_hop_node);
            rrc_stats.messages_discarded_no_slots++;
            generate_slot_assignment_failure_message(app_msg->dest_node_id);
            release_message(app_msg);
            return;
        ...
        printf("RRC: Allocated DU/GU slot %u for transmission to next hop %u\n",
               allocated_slot, next_hop_node);
    }
...
int receive_hello_packet_and_forward_to_tdma(uint8_t *hello_payload, size_t payload_len, uint8_t source_node)
{
    ...
    // NC slot is already assigned via seedex+round-robin during neighbor creation
    // NC info is sent at the source node's assigned NC slot
    NeighborState *neighbor = rrc_get_neighbor_state(source_node);
    if (!neighbor)
    {
        neighbor = rrc_create_neighbor_state(source_node);
    }

    uint8_t assigned_slot = neighbor ? neighbor->assignedNCSlot : 0;
    if (assigned_slot == 0)
    {
        printf("RRC: ⚠️ No NC slot assigned for node %u\n", source_node);
        olsr_nc_stats.tdma_nc_requests++;
        return -1;
    }
...
int receive_olsr_packet_for_nc(uint8_t *olsr_payload, size_t payload_len, uint8_t source_node)
{
    ...
    // NC slot is already assigned via seedex+round-robin
    // OLSR packets from this node will be sent on its assigned NC slot
    NeighborState *neighbor = rrc_get_neighbor_state(source_node);
    if (!neighbor)
    {
        neighbor = rrc_create_neighbor_state(source_node);
...
// Initialize RRC slot allocation table
void init_tdma_slot_table(void)
{
    for (int i = 0; i < 10; i++)
    {
        ...
        tdma_slot_table[i].is_nc_slot = (i >= 8); // Slots 8-9 are NC slots
        tdma_slot_table[i].collision_detected = false;
        tdma_slot_table[i].last_update = 0;
    }
    printf("RRC: Slot allocation table initialized (10 slots: 0-7 DU/GU, 8-9 NC)\n");
}

// RRC allocates DU/GU slots (0-7) based on priority
uint8_t rrc_allocate_du_gu_slot(uint8_t node_id, MessagePriority priority)
{
    // Allocate DU/GU slots (0-7) based on priority
    // Higher priority gets lower slot numbers (earlier transmission)
    uint8_t start_slot = 0;
    uint8_t end_slot = 7;

    // Priority-based slot range selection
    if (priority == PRIORITY_ANALOG_VOICE_PTT || priority == PRIORITY_DIGITAL_VOICE)
    {
        start_slot = 0; // Voice gets slots 0-3
        end_slot = 3;
    }
    else
    {
        start_slot = 4; // Data gets slots 4-7
        end_slot = 7;
    }

    // Find first available slot in priority range
    for (uint8_t slot = start_slot; slot <= end_slot; slot++)
    {
            ...
            tdma_slot_table[slot].assigned_node == node_id)
        {
            ...
            printf("RRC: Allocated DU/GU slot %u to node %u (priority %d)\n",
                   slot, node_id, priority);
            return slot;
        }
    }

    // If priority range full, try any available slot (0-7)
    for (uint8_t slot = 0; slot <= 7; slot++)
    {
        if (tdma_slot_table[slot].assigned_node == 0)
        {
            ...
            printf("RRC: Allocated fallback DU/GU slot %u to node %u\n", slot, node_id);
            return slot;
        }
    }

    printf("RRC: No DU/GU slots available for node %u\n", node_id);
    return 255; // No slot available
}

// Check if slot is available for allocation
bool rrc_check_slot_available(uint8_t node_id, MessagePriority priority)
{
    // Check if any DU/GU slot (0-7) is available
    for (uint8_t slot = 0; slot <= 7; slot++)
    {
        if (tdma_slot_table[slot].assigned_node == 0 ||
            tdma_slot_table[slot].assigned_node == node_id)
...
// Release slot allocation for a node
void rrc_release_slot(uint8_t node_id, uint8_t slot_id)
{
    if (slot_id < 10 && tdma_slot_table[slot_id].assigned_node == node_id)
    {
        tdma_slot_table[slot_id].assigned_node = 0;
        tdma_slot_table[slot_id].is_tx_slot = false;
        tdma_slot_table[slot_id].is_rx_slot = false;
        tdma_slot_table[slot_id].last_update = (uint32_t)time(NULL);

        printf("RRC: Released slot %u from node %u\n", slot_id, node_id);
    }
}

// Allocate or update slot assignments for a node
bool assign_tdma_slots(uint8_t node_id, bool tx_capable, bool rx_capable)
{
    printf("RRC: Assigning slots for node %u (TX=%s, RX=%s)\n",
           node_id, tx_capable ? "YES" : "NO", rx_capable ? "YES" : "NO");
    ...
    if (tx_capable)
    {
        // RRC allocates DU/GU slot (0-7) directly
        uint8_t allocated_slot = rrc_allocate_du_gu_slot(node_id, PRIORITY_DATA_1);

        if (allocated_slot != 255)
        {
            // Update neighbor's TX slots
            if (neighbor)
            {
                uint8_t byte_index = allocated_slot / 8;
                uint8_t bit_offset = allocated_slot % 8;
                neighbor->txSlots[byte_index] |= (1 << bit_offset);
            }

            printf("RRC: Assigned TX slot %u to node %u\n", allocated_slot, node_id);
            assignment_success = true;
        }
        else
        {
            printf("RRC: ⚠️ No DU/GU slots available for node %u\n", node_id);
            neighbor_stats.slot_conflicts_detected++;
    ...
    if (rx_capable)
    {
        printf("RRC: Node %u configured for RX capability\n", node_id);

        // Node can receive on any slot, mark RX capability
        if (neighbor)
        {
            // Mark that this node can receive (actual RX slots determined by others' TX)
    ...
    // NC slot is separately assigned via seedex+round-robin
    // NC info will be transmitted on the node's assigned NC slot (8 or 9)
    if (neighbor)
    {
        uint8_t nc_slot = rrc_assign_nc_slot(node_id);
        neighbor->assignedNCSlot = nc_slot;
        printf("RRC: Node %u assigned NC slot %u (for NC transmission)\n", node_id, nc_slot);
    }
...
// Print RRC slot allocation table
void print_tdma_slot_table(void)
{
    printf("\n=== RRC Slot Allocation Table ===\n");
    printf("Slot | Type  | Assigned Node | TX | RX | Collision | Last Update\n");
    printf("-----|-------|---------------|----|----|-----------|-------------\n");

    for (int i = 0; i < 10; i++)
    {
        const char *slot_type = (i >= 8) ? "NC" : "DU/GU";
        printf(" %2d  | %5s |      %3u      | %2s | %2s |     %2s    | %u\n",
    ...
    printf("===================================\n");
    printf("DU/GU slots (0-7): Data transmission\n");
    printf("NC slots (8-9): Network Control (assigned via seedex+round-robin)\n");
    printf("NC slot assignment: Each node gets its NC slot based on node ID\n\n");
}